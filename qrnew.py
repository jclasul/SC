...
for itemstock in stock.itertuples():
    ...
    for maat in maten.columns:
        for n in range(maten.loc[:,maat][0]):
            ...
            full_url_list = [url_start, IDnr, url_mid, VerkoopPrijs, url_mid2, maat, url_end]
            full_url = "".join(full_url_list)

            QRCODE = qrcode.make(URL).convert("L").resize((290,290))
            ...
            IMG_NAME = 'QRcodes/' + Merk + "_" + IDnr + "_" + str(n+1) + ".PNG"
